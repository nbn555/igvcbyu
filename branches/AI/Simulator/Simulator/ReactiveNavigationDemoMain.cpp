...
//YclopsReactiveNavInterface  myReactiveInterface;
...
void ReactiveNavigationDemoFrame::reloadRobotShape()
{
	try
	{
		...
		iniReactive->read_vector(robotName,"RobotModel_shape2D_xs",vector<double>(0), xs, true );
		iniReactive->read_vector(robotName,"RobotModel_shape2D_ys",vector<double>(0), ys, true );
...
void ReactiveNavigationDemoFrame::OnLoadPointsFileClick(wxCommandEvent& event)
{
	if(!reacNavObj){
		tryConstructReactiveNavigator();
	}
